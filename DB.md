...
重建表就是按照主键 ID 递增的顺序，把数据一行一行地从旧表中读出来再插入到新表中，重建表时 MySQL 会自动完成转存数据、交换表名、删除旧表的操作，线上操作会阻塞大量的线程增删改查的操作
...
自增主键的插入数据模式，不涉及到挪动其他记录，也**不会触发叶子节点的分裂**
...
在事务中加的锁，并不是不需要了就释放，而是**在事务中止或提交时自动释放**，这个就是两阶段锁协议。所以一般将更新共享资源（并发高）的 SQL 放到事务的最后执行，可以让其他线程尽量的减少等待时间
